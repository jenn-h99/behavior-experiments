protocol_name ='rev_prob_larger'
protocol_description = 'dfasdf'
...
large_reward_size = 70  # Volume(uL) of large reward for one random correct trial.
large_reward_steps = 1750   # Steps for 70 µL (70 µL = 1750  steps)
...
rule = core.Rule([highfreq,lowfreq], left_port, criterion, countdown_start, countdown)

# Initialize random seed to ensure proper randomization
np.random.seed()

# Select 20% of trials randomly to receive the large reward
# This will be checked against correct trials only
n_large_reward_trials = int(np.ceil(n_trials * 0.20))  # 20% of trials, rounded up
large_reward_trials = np.random.choice(n_trials, size=n_large_reward_trials, replace=False)
large_reward_trials = set(large_reward_trials)  # Convert to set for faster lookup

print('='*60)
print('LARGE REWARD CONFIGURATION:')
print('  Total trials: {}'.format(n_trials))
print('  Large reward trials: {} trials (20%)'.format(n_large_reward_trials))
print('  Large reward trial numbers: {}'.format(sorted(large_reward_trials)))
...
large_rewards_delivered = 0  # Track number of large rewards given
...
for trial in trials:
    ...
    if trial in large_reward_trials:
        # This trial is one of the selected large reward trials
        current_reward_size = large_reward_size
        current_reward_steps = large_reward_steps
        print('  >>> This is a LARGE REWARD trial! (Trial {})'.format(trial))
    else:
        ...
        while time.time() * 1000 < resp_window_end:
            ...
            if sum(lick_port_L._licks[(length_L-1):]) > 0:
                ...
                if np.random.rand() < p_rew:
                    ...
                    # Track large reward delivery
                    if current_reward_size == large_reward_size:
                        large_rewards_delivered += 1
                        print('  *** LARGE REWARD ({}uL) DELIVERED (#{}) ***'.format(
                            large_reward_size, large_rewards_delivered))
    ...
    licks_detected = ''  # Will indicate which ports recorded any licks in the entire trial.
...
data.total_reward = (total_reward_L + supp_reward_L + total_reward_R + supp_reward_R)
print('Total reward: {}uL'.format(data.total_reward))

# Report on large reward delivery
print('='*60)
print('LARGE REWARD SUMMARY:')
print('  Total large reward trials possible: {}'.format(n_large_reward_trials))
print('  Large rewards actually delivered: {}'.format(large_rewards_delivered))
print('  Missed large rewards: {} (incorrect responses)'.format(
    n_large_reward_trials - large_rewards_delivered))
print('='*60)
